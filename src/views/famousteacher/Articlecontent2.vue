...
<!-- huodong -->
	<div id="box" v-if="datalist" >
		<div id="content" v-for="data in datalist" :key="data.id"  >
			...
			<el-button @click="join(data.id)" class="canjia">参加</el-button>
		</div>
...
<script>
	export default{
		...
		methods:{
			join(id){
				console.log(id,localStorage.username)
				return this.axios.post('http://127.0.0.1:8080/Useractivity/joinActivity',{
					'id':id,
		...
		mounted() {
		return this.axios.post('http://127.0.0.1:8080/Activity/getAllstatu1Activity') //当页面加载时,返回
        .then(ret => {
